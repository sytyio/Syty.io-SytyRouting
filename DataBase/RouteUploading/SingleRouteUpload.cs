...
{
    public class SingleRouteUpload : BaseRouteUploader
    {
        ...
        public override async Task<int> UploadRouteAsync(string connectionString, string auxiliaryTable, string routeTable, Persona persona)
        {
            ...
            await using (var cmd = new NpgsqlCommand("UPDATE " + auxiliaryTable + " SET computed_route_2d = st_force2d(computed_route) WHERE persona_id = " + persona.Id + ";", connection))
            {
                await cmd.ExecuteNonQueryAsync();
            }

            await using (var cmd = new NpgsqlCommand("UPDATE " + auxiliaryTable + " SET is_valid_route = st_IsValidTrajectory(computed_route) WHERE persona_id = " + persona.Id + ";", connection))
            {
                await cmd.ExecuteNonQueryAsync();
            }

            await using (var cmd = new NpgsqlCommand("UPDATE " + auxiliaryTable + " SET is_valid_route = false WHERE st_IsEmpty(computed_route) AND persona_id = " + persona.Id + ";", connection))
            {
                await cmd.ExecuteNonQueryAsync();
            }

            await using (var cmd = new NpgsqlCommand("UPDATE " + auxiliaryTable + " SET route = computed_route::tgeompoint WHERE is_valid_route = true AND persona_id = " + persona.Id + ";", connection))
            {
                await cmd.ExecuteNonQueryAsync();
            }
            ...
            // $$;
            // ";
                              //UPDATE    persona_route   SET route = (SELECT route FROM    persona_route_aux   WHERE persona_id = 7) WHERE id = 7
