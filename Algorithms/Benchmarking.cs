...
namespace SytyRouting
{
    public class Benchmarking
    {
        ...
        private static List<Node> RoutingAlgorithmRunTime(IRoutingAlgorithm routingAlgorithm, Node origin, Node destination, byte[] transportModesSequence)
        {
            ...
            var xympRoute = routingAlgorithm.NodeRouteToLineStringMSeconds(0,0,0,0,route, TimeSpan.Zero); // <- debug: verify start and end coordinates
            stopWatch.Stop();
        ...
        private static void MultipleRandomSourceTargetRouting(Graph graph, IRoutingAlgorithm algorithm1, IRoutingAlgorithm algorithm2, byte[] transportModesSequence, int numberOfRuns)
        {
            ...
            for(int i = 0; i < numberOfRuns; i++)
            {
                ...
                var xympRoute1 = algorithm1.NodeRouteToLineStringMSeconds(0,0,0,0,route1, TimeSpan.Zero); // <- debug: verify start and end coordinates
                ...
                var xympRoute2 = algorithm2.NodeRouteToLineStringMSeconds(0,0,0,0,route2, TimeSpan.Zero); // <- debug: verify start and end coordinates
